...
function wordsToLetters() {
	// Title.
	charming(title.el);
	title.letters = [].slice.call(title.el.querySelectorAll('span'));
	// Contact.
	charming(contact.el);
	contact.letters = [].slice.call(contact.el.querySelectorAll('span'));
	// Menu items (code mode).
	menuCodeItemLetters = [];
	[].slice.call(menu.code.items).forEach(function(item) {
		charming(item);
		menuCodeItemLetters.push([].slice.call(item.querySelectorAll('span')));
	});
}
...
function switchMode(ev) {
	...
	// mode: design||code.
	mode = ev.target === switchModeCtrls.code ? 'code' : 'design';
	...
	switchModeCtrls[mode === 'code' ? 'design' : 'code'].classList.remove('switch__item--current');
	switchModeCtrls[mode].classList.add('switch__item--current');
...
function switchContent() {
	...
	switchCtrls.classList.remove('mode--' + (mode === 'code' ? 'design' : 'code'));
	switchCtrls.classList.add('mode--' + mode);
...
function switchToCode() {
	const hideDesign = function(target, callback) {
		...
		if( typeof target === 'string' ) {
			animeOpts.targets = ev.target.el || ev.target;
	...
	showCode = function(target) {
		...
		if( ev.letters ) {
			animateLetters(ev.target.letters, 'in', {
				begin: function() {
					ev.target.el.style.opacity = 1;
					ev.target.el.style.transform = 'none';
				}
			});
		} else {
			el.style.opacity = 1;
	...
	// Menu links:
	hideDesign(menu['design'].items, function() {
		menu['design'].wrapper.style.display = 'none';
			
		animateLetters(menuCodeItemLetters, 'in', {
			delay: function(t,i) {
				return i*30
			},
			begin: function() {
				menu['code'].wrapper.style.display = 'block';
			}
		});
	});
}
...
function switchToDesign() {
	const showDesign = function(target) {
		...
		if( typeof target === 'string' ) {
			...
			animeOpts.begin = function() {
				if( ev.target.letters !== undefined ) {
					ev.target.letters.forEach(function(letter) {
	...
	// Title:
	animateLetters(title.letters, 'out', {
		complete: function() {
			showDesign('title');
		}
	});
	// Contact:
	animateLetters(contact.letters, 'out', {
		complete: function() {
			showDesign('contact');
		}
	});
	// Menu ctrl:
	menuCtrl.style.opacity = 0;
	showDesign('menuCtrl');
	// Menu links:
	animateLetters(menuCodeItemLetters, 'out', {
		delay: function(t,i,c) {
			return (c-i-1)*10;
		},
		duration: 20,
		complete: function() {
			menu['code'].wrapper.style.display = 'none';
			menu['design'].wrapper.style.display = 'block';
			showDesign(menu['design'].items);
		}
	});
}
